...
struct vbva_buffer {
	...
	uint32_t record_first_index;
	uint32_t record_free_index;

	/* Space to leave free when large partial records are transferred. */
	uint32_t partial_write_tresh;
...
struct vbva_modehint {
	...
	uint32_t cx;
	uint32_t cy;
	uint32_t bpp;		/* Which has never been used... */
	uint32_t display;
