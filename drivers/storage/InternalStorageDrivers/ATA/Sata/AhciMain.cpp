...
bool IsAhciController(P_PCI_DEVICE_OBJECT PDEV){
    ...
    if((Config.Header.BaseClass == 0x01) && (Config.Header.SubClass == 0x06)){
        return true;
    }
...
void InitializeAhciDevice(P_PCI_DEVICE_OBJECT PDEV){
    ...
    uint64_t BoardID = LouKeHalLinuxPciCheckForCompatibleConfiguration(&Config, AhciPciTable);
    PATA_PORT PortInfo = (PATA_PORT)LouMalloc(sizeof(ATA_PORT));//global Port Variable
    GetAhciDevicePortInfo(BoardID, PortInfo);
    PAHCI_MEMORY_REGISTERS Host;
    PAHCI_DRIVER_EXTENDED_OBJECT Ext = (PAHCI_DRIVER_EXTENDED_OBJECT)LouMalloc(sizeof(AHCI_DRIVER_EXTENDED_OBJECT));
    PDEV->DeviceExtendedObject = (uintptr_t)Ext;
    UNUSED int ABAR = 5;

    if(IsAppleBios(&Config)){
        AppleBiosSetup(PDEV);
    }
    ...
    if((BoardID == BoardAhciMcp65) && 
    ((Config.Header.RevisionID == 0xA1) || (Config.Header.RevisionID == 0xA2))){
        PortInfo->HFlags |= AHCI_HFLAG_NO_MSI;
    }
    if((BoardID == BoardAhciSb700) && (Config.Header.RevisionID >= 0x40)){
        PortInfo->HFlags &= ~(AHCI_HFLAG_IGN_SERR_INTERNAL);
    }

    PortInfo->HostMmio = LouKeHalGetPciVirtualBaseAddress(&Config, ABAR); 
    Host = (PAHCI_MEMORY_REGISTERS)PortInfo->HostMmio;
